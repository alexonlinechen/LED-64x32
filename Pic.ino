...
void DrawPic(const unsigned char pic[], bool BgCover, int pic_x, int pic_y, int pic_w, int pic_h)
{
    ...
    for (y = 0; y < pic_h; y++)
    {
        for (x = 0; x < pic_w; x++)
        {
            ...
            b = pic[3 * (y * pic_w + x) + 2];
            if (BgCover || r || g || b)
                display.drawPixel(pic_x+x,pic_y+y,display.color565(r,g,b));
...
void SetEmoji(int emoji_y, int emoji_x) // 1~10
{
    ...
    DrawPic(curremoji,true,0+GlobalX,0+GlobalY,8,8);
}
...
* 返回值：无
**********************************************************/
void SetEmojiXY(int emoji_y, int emoji_x, int x, int y) // 1~10
{
    int i;
    if (emoji_y <= 5)
    {
